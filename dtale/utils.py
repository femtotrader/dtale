...
def get_host(host=None):
    if host is None:
        if os.environ.get('JUPYTERHUB_USER_POD_SVC'):
            return os.environ['JUPYTERHUB_USER_POD_SVC']
        else:
...
def build_shutdown_url(port, host=None):
    """
    ...
    """
    return '{}/shutdown'.format(build_url(port, host=host))
